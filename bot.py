...
async def send_audio(message: Message, info_dict, audio_file):
    ...
    if Config.AUDIO_THUMBNAIL == "No":
        ...
        thumbnail_file = wget.download(Config.AUDIO_THUMBNAIL)
...
async def send_video(message: Message, info_dict, video_file):
    ...
    if Config.VIDEO_THUMBNAIL == "No":
        ...
        thumbnail_file = wget.download(Config.VIDEO_THUMBNAIL)
    webpage_url = info_dict['webpage_url']
    title = s2tw(info_dict['title'])
    caption = f"<b><a href=\"{webpage_url}\">{title}</a></b>"
    duration = int(float(info_dict['duration']))
    width, height = get_resolution(info_dict)
    await message.reply_video(video_file, caption=caption, duration=duration,
...
def get_resolution(info_dict):
    if {"width", "height"} <= info_dict.keys():
        ...
        height = int(info_dict.get('height', 360))
        width = {1080: 1920, 720: 1280, 480: 854, 360: 640, 240: 426}.get(height, 640)
    return (width, height)
